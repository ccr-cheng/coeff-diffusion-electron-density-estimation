...
class DensityCollator:
    ...
    def __call__(self, batch):
        g, density, grid_coord, infos = zip(*batch)
        g = Batch.from_data_list(g)

        if self.n_samples is None:
            density = pad_sequence(density, batch_first=True, padding_value=-1)
            grid_coord = pad_sequence(grid_coord, batch_first=True, padding_value=0.)
            return g, density, grid_coord, infos

        sampled_density, sampled_grid = [], []
        for d, coord in zip(density, grid_coord):
...
class DensityVoxelCollator:
    def __call__(self, batch):
        ...
        for den, grid, shape in zip(density, grid_coord, shapes):
        ...
        density = torch.stack(padded_density, dim=0)
